...
})(document.currentScript, (script, kansenTagName, meowfficerTagName) => {
    const tagName = script.dataset.tagname || 'fleet-formation';
    const FLEET_TYPES = ['surface', 'submarine'];
    const kansenConstructor = customElements.get(kansenTagName);
    const meowfficerConstructor = customElements.get(meowfficerTagName);
    kansenTagName = kansenTagName.replace(/[a-z]/g, (c) => {
        return String.fromCharCode(c.charCodeAt(0) & ~32);
    });
    meowfficerTagName = meowfficerTagName.replace(/[a-z]/g, (c) => {
        return String.fromCharCode(c.charCodeAt(0) & ~32);
    });
    ((component) => {
        ...
        constructor() {
            ...
            this.svg = this.createFrame((index) => {
                const list = this.createKansenList(this.submarine ? 'submarine' : index < 3 ? 'main' : 'vanguard');
                this.renderDialogContents(dialog, list.map((kansen) => {
                ...
                this.renderDialogContents(dialog, list.map((meowfficer) => {
                    const span = document.createElement('span');
        ...
        createKansenList(type) {
            return kansenConstructor.list(type);
        }
        createMeowfficerList() {
        ...
        parseChildren() {
            ...
            for (const child of this.children) {
                if (child.tagName === kansenTagName) {
                    switch (child.fleet) {
                        case 'main':
                            result.mains.push(child);
                            check.mains.push(i);
                            break;
                        case 'vanguard':
                            result.vanguards.push(child);
                            check.vanguards.push(i);
                            break;
                        case 'submarine':
                            result.submarines.push(child);
                            check.mains.push(i);
                            break;
                    }
                    result.kansen.push(child);
                    ++i;
                }
                else if (child.tagName === meowfficerTagName) {
                    result.meowfficers.push(child);
                }
            }
        ...
        updateCanvas() {
            ...
            img.src = 'data:image/svg+xml;base64,' +
                btoa(unescape(encodeURIComponent(this.svg.outerHTML.replace(/<svg/, `<svg xmlns="http://www.w3.org/2000/svg" style="${inStyle}"`))));
        }
        get edit() {
        ...
        createFrame(openKansenModal, openMeowfficerModal) {
            const back = document.createElementNS('http://www.w3.org/2000/svg', 'path');
            back.setAttributeNS(null, 'd', 'm 20,5 h 1295 c 2.77,0 5,2.23 5,5 v 180 c 0,2.77 -2.23,5 -5,5 H 20 c -2.77,0 -5,-2.23 -5,-5 V 10 c 0,-2.77 2.23,-5 5,-5 z');
