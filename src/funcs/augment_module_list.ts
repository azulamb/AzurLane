...
function DrawAugmentModuleList(parent: HTMLElement) {
	Promise.all([
		...
		const SKILL: { [keys: string]: { name: string; text: string } } = {
			...
			k2: {
				name: '先導攻撃-航空',
				text: '自身が戦闘中1回目の航空攻撃を行う場合、味方主力艦隊の航空が10秒間20アップ（「先導攻撃-航空」装備効果は1回しか加算されない）',
			},
			k3: { name: '空中戦闘技術', text: '自身の艦載機が敵機5機を撃墜した場合に1度だけ、自身の航空が20アップ' },
			k4: {
				name: '弾薬調整-徹甲',
				text: '自身の1番目の兵装枠に装備されている兵装の攻撃が装甲タイプが重装甲の敵に与えるダメージが5%アップし、軽装甲の敵に与えるダメージが5%ダウン',
			},
			k5: {
				name: '先導攻撃-火力',
				text: '自身が戦闘中1回目の遠隔砲撃攻撃を行う場合、味方主力艦隊の火力が10秒間20アップ（「先導攻撃-火力」装備効果は1回しか加算されない）',
			},
